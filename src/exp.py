...
class Experiament:
    ...
    spans: List[str]
    metric: Callable[[torch.Tensor, pd.DataFrame,
                      HookedTransformer], torch.Tensor]
    seed: int
    ...
    def get_span_masks(self, row: pd.DataFrame, model: HookedTransformer, dtype: str):
        ...
        masks = torch.zeros(
            (len(self.spans), 1, row['length'], model.cfg.d_model), dtype=dtype).to("cuda")
        ...
        for span_idx in range(len(self.spans[:-1])):
            ...
            lengths[span_idx] = max(
                row[self.spans[span_idx + 1]] - row[self.spans[span_idx]], 1)
            indices.append((row[self.spans[span_idx]],
                           row[self.spans[span_idx + 1]]))
...
class GreaterThan(Experiament):
    ...
    def __init__(self, exp_name,  model_name, model_path,  ablation_type, clean_dataset_path, counter_dataset_path, spans,  metric, seed):
        super().__init__(exp_name=exp_name, model_name=model_name, model_path=model_path, ablation_type=ablation_type,
                         clean_dataset_path=clean_dataset_path, counter_dataset_path=counter_dataset_path, spans=spans, metric=metric, seed=seed)
    ...
    def get_mean_activations(self, model: HookedTransformer, row: pd.DataFrame, index: int,  seed: int, sample_ablation_size: int = 16) -> ActivationCache:
        ...
        if sample_ablation_size > 1:  # mean ablation
            df_counter = df_counter[(df_counter["split"] == "ablation") & (
                ~(df_counter["label"] == label))]
            df_counter = df_counter.sample(
                n=sample_ablation_size, random_state=seed)
        ...
        def forward_cache_hook(act, hook):
            mean_activations[hook.name] = torch.unsqueeze(
                act.detach().clone().mean(0), 0)  # batch seq d

        kq_pref = "rot_" if model.cfg.positional_embedding_type == "rotary" else ""
        nodes_names = ("mlp_out", "result", "hook_v",
                       f"hook_{kq_pref}q", f"hook_{kq_pref}k", "resid_pre", "attn_out", "pattern")
...
class IOI(Experiament):
    ...
    def create_datasets(self) -> Union[Tuple[pd.DataFrame, pd.DataFrame], Tuple[pd.DataFrame, None]]:
        ...
        df_clean = pd.read_csv(self.clean_dataset_path)
        df_clean = df_clean[(df_clean["split"] == "circuit") & (
            df_clean["top_answer"] == df_clean["correct_token"])]
        df_counter = None
        df_counter = pd.read_csv(self.counter_dataset_path).loc[df_clean.index]
    ...
    def get_mean_activations(self, model: HookedTransformer, row, index, seed, sample_ablation_size=16):
        ...
        if sample_ablation_size > 1:  # mean ablation
            df = df[(df["split"] == "ablation") & (~(df["label"] == label)) & (
                df["prompt_id"] == row["prompt_id"])]
        ...
        def forward_cache_hook(act, hook):
            if hook.name not in mean_activations:
                mean_activations[hook.name] = act.detach(
                ) / sample_ablation_size  # batch seq d
            else:
                mean_activations[hook.name] += act.detach() / \
                    sample_ablation_size  # batch seq d
    ...
    def get_df_for_eval(self, n_examples) -> pd.DataFrame:
        ...
        df = df[(df["split"] == "eval") & (df["top_answer"] == df["correct_token"])
                & df['correct_token'].apply(lambda x: x.strip().isalpha())]
        return df.sample(n=min(n_examples, df.shape[0]), random_state=self.seed)
...
class WinoBias(Experiament):
    ...
    def get_mean_activations(self, model: HookedTransformer, row, index,  seed, sample_ablation_size=16):
        ...
        for file in self.counter_dataset_path:
            ...
            df = df[(df["id"] == prompt_id) & (df["split"] == "ablation")].sample(
                n=sample_ablation_size//4, random_state=seed)
            df_list.append(df)
        ...
        def forward_cache_hook(act, hook):
            mean_activations[hook.name] = torch.unsqueeze(
                act.detach().mean(0), 0)  # batch seq d
...
def logit_diff(logits: torch.Tensor, row: pd.DataFrame, model: HookedTransformer) -> torch.Tensor:
    ...
    correct_token = model.to_tokens(
        row["correct_token"], prepend_bos=False)[0, 0].item()
    wrong_token = model.to_tokens(
        row["wrong_token"], prepend_bos=False)[0, 0].item()
...
def prob_diff(logits: torch.Tensor, row: pd.DataFrame, model: HookedTransformer) -> torch.Tensor:
    ...
    prob_diff = probs[:, correct_year + 1:].sum() - \
        probs[:, : correct_year + 1].sum()
