...
class EAPResults:
    ...
    def update_score(self, edge: Edge, scores: Dict[str, torch.Tensor]):
        ...
        if edge not in self.results:
            self.results[edge] = EdgeScore(avg_score=scores["avg"], sum_score=scores["sum"], sum_abs_pos_score=scores["sum_abs_pos"],
                                           sum_abs_exp_score=scores["sum_abs_exp"], max_abs_score=scores["max_abs"])
        else:
    ...
    def get_average_scores(self):
        for edge in self.results.keys():
            try:
                self.results[edge].avg_score = np.mean(
                    self.results[edge].avg_score, axis=0)
                self.results[edge].sum_score = np.mean(
                    self.results[edge].sum_score, axis=0)
                self.results[edge].sum_abs_pos_score = np.mean(
                    self.results[edge].sum_abs_pos_score, axis=0)
                self.results[edge].sum_abs_exp_score = np.mean(
                    self.results[edge].sum_abs_exp_score, axis=0)
                self.results[edge].max_abs_score = np.mean(
                    self.results[edge].max_abs_score, axis=0)
            except Exception as e:
                print("Error in edge:", edge,
                      "with scores:", self.results[edge])
                raise e
...
def position_aware_edge_attribution_patching(
    ...
    random_indices = np.random.choice(
        range(len(ds_clean)), size=dataset_size, replace=False)
...
# Select the same rows from both datasets
    ...
    results = EAPResults(dataset_name=exp.clean_dataset_path,
                         ...
                         counterfactual=ds_counterfactual["prompt"].to_list(
                         ) if exp.ablation_type == "counterfactual" else [],
    ...
    for index, row in tqdm(ds_clean.iterrows(), total=ds_clean.shape[0]):
        ...
        if exp.ablation_type == "mean":
            mean_activations, mean_df = exp.get_mean_activations(
                model=model, row=row, index=index, seed=exp.seed)
            del mean_df

        masks, lengths, indices = exp.get_span_masks(
            row, model, dtype=model.cfg.dtype)

        clean_cache, counter_cache, clean_grad_cache = get_hooked_activations(
            model, clean=row, counterfactual=ds_counterfactual.loc[index], exp=exp)

        # activations
        # heads output
        clean_head_act = split_layers_and_heads(clean_cache.stack_head_results(
        ), model=model).clone().to("cuda")  # layer head batch seq d_model
        clean_head_results_act = split_layers_and_heads_for_heads_results(
            clean_cache.stack_head_results(), model=model).clone().to("cuda")  # layer batch seq head d_model
        # mlp output
        clean_mlp_act = clean_cache.stack_activation(
            activation_name="mlp_out").clone().to("cuda")  # layer batch seq d_model
        # residual 0 output
        clean_resid_pre_act = clean_cache['blocks.0.hook_resid_pre'].clone().to(
            "cuda")  # batch seq d_model

        if exp.ablation_type == "mean":
            counter_head_act = split_layers_and_heads(
                mean_activations.stack_head_results(), model=model).clone().to("cuda")
            counter_mlp_act = mean_activations.stack_activation(
                activation_name="mlp_out").to("cuda").clone().clone()  # layer batch seq d_model
            counter_resid_pre_act = mean_activations['blocks.0.hook_resid_pre'].to(
                "cuda").clone().clone()  # batch seq d_model
        elif exp.ablation_type == "zero":
            counter_head_act = torch.zeros_like(
                clean_head_act, dtype=model.cfg.dtype)
            counter_mlp_act = torch.zeros_like(
                clean_mlp_act, dtype=model.cfg.dtype)
            counter_resid_pre_act = torch.zeros_like(
                clean_resid_pre_act, dtype=model.cfg.dtype)
        elif exp.ablation_type == "counterfactual":
            counter_head_act = split_layers_and_heads(
                counter_cache.stack_head_results(), model=model).clone().to("cuda")
            counter_mlp_act = counter_cache.stack_activation(
                activation_name="mlp_out").clone().to("cuda")  # layer batch seq d_model
            counter_resid_pre_act = counter_cache['blocks.0.hook_resid_pre'].clone().to(
                "cuda")  # batch seq d_model

        clean_v_act = clean_cache.stack_activation(
            activation_name="v").clone().to("cuda")  # layer batch seq head d_head

        clean_k_act = clean_cache.stack_activation(
            activation_name=kq_pref + "k").clone().to("cuda")  # layer batch seq head d_head
        clean_q_act = clean_cache.stack_activation(
            activation_name=kq_pref + "q").clone().to("cuda")  # layer batch seq head d_head

        # attn pattern
        clean_attn_pattern = clean_cache.stack_activation(activation_name="pattern").clone().to("cuda").to(
            model.cfg.dtype)  # layer batch head seq seq #For numerical reaosn the pattern is float32

        clean_attn_scores = clean_cache.stack_activation(activation_name="attn_scores").clone().to(
            "cuda").to(model.cfg.dtype)  # layer batch head seq seq #For numerical reaosn the pattern is float32
        # gradients
        clean_z_scores = clean_cache.stack_activation(
            activation_name="z").clone().to("cuda")  # layer batch seq head d_head
        grad_q = clean_grad_cache.stack_activation(
            activation_name="q_input").clone().to("cuda")  # layer batch seq head dim
        grad_q = einops.rearrange(
            grad_q, "layer batch seq n_heads d -> layer n_heads batch seq d")

        grad_k = clean_grad_cache.stack_activation(
            activation_name="k_input").clone().to("cuda")  # layer batch seq head dim
        grad_k = einops.rearrange(
            grad_k, "layer batch seq n_heads d -> layer n_heads batch seq d")

        grad_v = clean_grad_cache.stack_activation(
            activation_name="v_input").clone().to("cuda")  # layer batch seq head dim
        grad_v = einops.rearrange(
            grad_v, "layer batch seq n_heads d -> layer n_heads batch seq d")

        grad_mlp = clean_grad_cache.stack_activation(
            activation_name="mlp_in").clone().to("cuda")  # layer batch seq dim
        grad_resid = clean_grad_cache.stack_activation(
            activation_name="resid_post").clone().to("cuda")  # layer batch seq dim
        grad_heads_output = clean_grad_cache.stack_activation(
            activation_name="result").clone().to("cuda")  # layer batch seq head d_head
        ...
        for i in tqdm(range(len(nodes))):
            ...
            if node.node_type == "m":
                ...
                # batch seq d
                counter = counter_mlp_act[node.layer_idx].clone()
            elif node.node_type == "h":
                clean = clean_head_act[node.layer_idx,
                                       node.head_idx].clone()  # batch seq d
                counter = counter_head_act[node.layer_idx,
                                           node.head_idx].clone()
            else:
            ...
            attribute_score_mlp = temp2.unsqueeze(1).repeat(
                1, len(exp.spans), 1, 1, 1)  # layer span batch seq d model
            attribute_score_mlp_masked = torch.where(masks.unsqueeze(0).repeat(
                attribute_score_mlp.shape[0], 1, 1, 1, 1) == 1, attribute_score_mlp, 0)
            attribute_score_mlp_avg = (attribute_score_mlp_masked.sum(dim=(2, 3, 4)) / lengths.unsqueeze(
                0).repeat(attribute_score_mlp.shape[0], 1)).detach().to(torch.float32).cpu().numpy()
            attribute_score_mlp_sum = (attribute_score_mlp_masked.sum(
                dim=(2, 3, 4))).detach().to(torch.float32).cpu().numpy()
            attribute_score_mlp_sum_abs_pos = (attribute_score_mlp_masked.sum(
                4).abs().sum(dim=(2, 3))).detach().to(torch.float32).cpu().numpy()
            attribute_score_mlp_sum_abs_exp = (attribute_score_mlp_masked.sum(
                (3, 4)).abs().sum(dim=2)).detach().to(torch.float32).cpu().numpy()
            attribute_score_mlp_max_abs = (attribute_score_mlp_masked.sum(
                4).abs().max(3).values.sum(2)).detach().to(torch.float32).cpu().numpy()

            del attribute_score_mlp_masked, attribute_score_mlp,

            attribute_score_resid = (grad_resid[-1] * (counter - clean)).unsqueeze(
                0).repeat(len(exp.spans), 1, 1, 1)  # span batch seq d model
            attribute_score_resid_masked = torch.where(
                masks == 1, attribute_score_resid, 0)
            attribute_score_resid_avg = (attribute_score_resid_masked.sum(
                dim=(1, 2, 3)) / lengths).detach().to(torch.float32).cpu().numpy()
            attribute_score_resid_sum = (attribute_score_resid_masked.sum(
                dim=(1, 2, 3))).detach().to(torch.float32).cpu().numpy()
            attribute_score_resid_sum_abs_pos = (attribute_score_resid_masked.sum(
                3).abs().sum(dim=(1, 2))).detach().to(torch.float32).cpu().numpy()
            attribute_score_resid_sum_abs_exp = (attribute_score_resid_masked.sum(
                (2, 3)).abs().sum(dim=(1))).detach().to(torch.float32).cpu().numpy()
            attribute_score_resid_max_abs = (attribute_score_resid_masked.sum(
                3).abs().max(2).values.sum(1)).detach().to(torch.float32).cpu().numpy()

            del attribute_score_resid, attribute_score_resid_masked

            attribute_score_q = (grad_q[node.layer_idx:] * (counter - clean)).unsqueeze(
                3).repeat(1, 1, len(exp.spans), 1, 1, 1)  # layer haed span batch seq d_model
            attribute_score_q_masked = torch.where(masks.unsqueeze(0).unsqueeze(1).repeat(
                attribute_score_q.shape[0], model.cfg.n_heads, 1, 1, 1, 1) == 1, attribute_score_q, 0)
            attribute_score_q_avg = (attribute_score_q_masked.sum(dim=(3, 4, 5)) / lengths.unsqueeze(0).unsqueeze(
                1).repeat(attribute_score_q.shape[0], model.cfg.n_heads, 1)).detach().to(torch.float32).cpu().numpy()
            attribute_score_q_sum = (attribute_score_q_masked.sum(
                dim=(3, 4, 5))).detach().to(torch.float32).cpu().numpy()
            attribute_score_q_sum_abs_pos = (attribute_score_q_masked.sum(
                5).abs().sum(dim=(3, 4))).to(torch.float32).detach().cpu().numpy()
            attribute_score_q_sum_abs_exp = (attribute_score_q_masked.sum(
                (4, 5)).abs().sum(dim=(3))).detach().to(torch.float32).cpu().numpy()
            attribute_score_q_max_abs = (attribute_score_q_masked.sum(5).abs().max(
                4).values.sum(3)).detach().to(torch.float32).cpu().numpy()

            del attribute_score_q, attribute_score_q_masked

            attribute_score_k = (grad_k[node.layer_idx:] * (counter - clean)).unsqueeze(
                3).repeat(1, 1, len(exp.spans), 1, 1, 1)  # layer haed span batch seq d_model
            attribute_score_k_masked = torch.where(masks.unsqueeze(0).unsqueeze(1).repeat(
                attribute_score_k.shape[0], model.cfg.n_heads, 1, 1, 1, 1) == 1, attribute_score_k, 0)
            attribute_score_k_avg = (attribute_score_k_masked.sum(dim=(3, 4, 5)) / lengths.unsqueeze(0).unsqueeze(
                1).repeat(attribute_score_k.shape[0], model.cfg.n_heads, 1)).detach().to(torch.float32).cpu().numpy()
            attribute_score_k_sum = (attribute_score_k_masked.sum(
                dim=(3, 4, 5))).detach().to(torch.float32).cpu().numpy()
            attribute_score_k_sum_abs_pos = (attribute_score_k_masked.sum(
                5).abs().sum(dim=(3, 4))).detach().to(torch.float32).cpu().numpy()
            attribute_score_k_sum_abs_exp = (attribute_score_k_masked.sum(
                (4, 5)).abs().sum(dim=(3))).detach().to(torch.float32).cpu().numpy()
            attribute_score_k_max_abs = (attribute_score_k_masked.sum(5).abs().max(
                4).values.sum(3)).detach().to(torch.float32).cpu().numpy()

            del attribute_score_k, attribute_score_k_masked

            attribute_score_v = (grad_v[node.layer_idx:] * (counter - clean)).unsqueeze(
                3).repeat(1, 1, len(exp.spans), 1, 1, 1)  # layer haed span batch seq d_model
            attribute_score_v_masked = torch.where(masks.unsqueeze(0).unsqueeze(1).repeat(
                attribute_score_v.shape[0], model.cfg.n_heads, 1, 1, 1, 1) == 1, attribute_score_v, 0)
            attribute_score_v_avg = (attribute_score_v_masked.sum(dim=(3, 4, 5)) / lengths.unsqueeze(0).unsqueeze(
                1).repeat(attribute_score_v.shape[0], model.cfg.n_heads, 1)).detach().to(torch.float32).cpu().numpy()
            attribute_score_v_sum = (attribute_score_v_masked.sum(
                dim=(3, 4, 5))).detach().to(torch.float32).cpu().numpy()
            attribute_score_v_sum_abs_pos = (attribute_score_v_masked.sum(
                5).abs().sum(dim=(3, 4))).detach().to(torch.float32).cpu().numpy()
            attribute_score_v_sum_abs_exp = (attribute_score_v_masked.sum(
                (4, 5)).abs().sum(dim=(3))).detach().to(torch.float32).cpu().numpy()
            attribute_score_v_max_abs = (attribute_score_v_masked.sum(5).abs().max(
                4).values.sum(3)).detach().to(torch.float32).cpu().numpy()
            ...
            edge = Edge(upstream_layer_idx=node.layer_idx,
                        ...
                        downstream_full_name=utils.get_act_name(
                            "resid_post", model.cfg.n_layers-1),
                        span_upstream=None,
            ...
            del attribute_score_mlp_avg, attribute_score_mlp_sum, attribute_score_mlp_sum_abs_pos, attribute_score_mlp_sum_abs_exp, attribute_score_mlp_max_abs, attribute_score_resid_avg, attribute_score_resid_sum, attribute_score_resid_sum_abs_pos, attribute_score_resid_sum_abs_exp, attribute_score_resid_max_abs, attribute_score_q_avg, attribute_score_q_sum, attribute_score_q_sum_abs_pos, attribute_score_q_sum_abs_exp, attribute_score_q_max_abs, attribute_score_k_avg, attribute_score_k_sum, attribute_score_k_sum_abs_pos, attribute_score_k_sum_abs_exp, attribute_score_k_max_abs, attribute_score_v_avg, attribute_score_v_sum, attribute_score_v_sum_abs_pos, attribute_score_v_sum_abs_exp, attribute_score_v_max_abs, clean, counter
        ...
        for upstream_span_idx in tqdm(range(len(exp.spans)-1)):
            len_upstream_span = indices[upstream_span_idx][1] - \
                indices[upstream_span_idx][0]
            ...
            for layer_idx in range(model.cfg.n_layers):
                ...
                pattern = clean_attn_pattern[layer_idx].clone()
                # key_pos head_index d_head
                patched_v = clean_v_act[layer_idx].clone()
                if exp.ablation_type == "mean":
                    counter = mean_activations[utils.get_act_name(
                        "v", layer_idx)].clone()
                elif exp.ablation_type == "zero":
                    counter = torch.zeros_like(
                        clean_v_act[layer_idx], dtype=model.cfg.dtype)
                elif exp.ablation_type == "counterfactual":
                    counter = counter_cache[utils.get_act_name(
                        "v", layer_idx)].clone()
                ...
                q = clean_q_act[layer_idx].clone()
                # key_pos head_index d_head
                patched_k = clean_k_act[layer_idx].clone()
                if exp.ablation_type == "mean":
                    counter = mean_activations[utils.get_act_name(
                        kq_pref + "k", layer_idx)].clone()
                elif exp.ablation_type == "zero":
                    counter = torch.zeros_like(
                        clean_k_act[layer_idx], dtype=model.cfg.dtype)
                elif exp.ablation_type == "counterfactual":
                    counter = counter_cache[utils.get_act_name(
                        kq_pref + "k", layer_idx)].clone()
                ...
                patched_attn_scores = calculate_attention_scores(
                    q, patched_k, attn_scale)

                orig_attn_scores = clean_attn_scores[layer_idx].clone()
                patched_attn_scores = torch.where(
                    orig_attn_scores != torch.tensor(-torch.inf), patched_attn_scores,  torch.tensor(-torch.inf))
                ...
                z_patched_k = calculate_z_scores(
                    clean_v_act[layer_idx], patched_pattern)
                ...
                for i in range(z_patched_v.shape[0]):
                    result_patched_v.append(calculate_heads_output(
                        model, layer_idx, z_patched_v[i]))
                ...
                for i in range(z_patched_k.shape[0]):
                    result_patched_k.append(calculate_heads_output(
                        model, layer_idx, z_patched_k[i]))
                if len(result_patched_k) > 0:
                    result_patched_k = torch.stack(result_patched_k, dim=0)
                else:
                    result_patched_k = results_clean

                attribute_score_v = grad * \
                    (result_patched_v - results_clean)  # batch seq n_heads d
                attribute_score_k = grad * \
                    (result_patched_k - results_clean)  # batch n_heads d
                ...
                for head_idx in range(model.cfg.n_heads):
                    for downstream_span_idx in range(upstream_span_idx + 1, len(exp.spans)-1):
                        ...
                        if indices[downstream_span_idx][1] - indices[downstream_span_idx][0] > 0:
                            attribute_score = attribute_score_v[:, indices[downstream_span_idx]
                                                                [0]:indices[downstream_span_idx][1], head_idx]
                            scores_v = {
                        ...
                        edge = Edge(upstream_layer_idx=layer_idx,
                                    ...
                                    is_crossing=True)
                        if indices[downstream_span_idx][1] - indices[downstream_span_idx][0] > 0:
                            attribute_score = attribute_score_k[:, indices[downstream_span_idx]
                                                                [0]:indices[downstream_span_idx][1], head_idx]
                            scores_k = {
                                "avg": attribute_score.sum(-1).mean().detach().to(torch.float32).cpu().numpy(),
                                "sum": attribute_score.sum().detach().to(torch.float32).cpu().numpy(),
                    ...
                    attribute_score = attribute_score_v[:, indices[downstream_span_idx]
                                                        [0]:indices[downstream_span_idx][1], head_idx]
                    attribute_score = [attribute_score[i, i:]
                                       for i in range(attribute_score.shape[1])]
        ...
        for downstream_span_idx in tqdm(range(len(exp.spans)-1)):
            for layer_idx in range(model.cfg.n_layers):
                ...
                if exp.ablation_type == "mean":
                    counter_q = mean_activations[utils.get_act_name(
                        kq_pref + "q", layer_idx)].clone()
                elif exp.ablation_type == "zero":
                    counter_q = torch.zeros_like(clean_q_act[layer_idx])
                elif exp.ablation_type == "counterfactual":
                    counter_q = counter_cache[utils.get_act_name(
                        kq_pref + "q", layer_idx)].clone()

                counter_attn_scores = calculate_attention_scores(
                    counter_q, k, attn_scale)
                ...
                for upstream_span_idx in range(downstream_span_idx):
                    ...
                    patched_attn_scores = patched_attn_scores.repeat(
                        len_upstream_span, 1, 1, 1)
                    for i in range(len_upstream_span):
                        patched_attn_scores[i, :, indices[downstream_span_idx][0]: indices[downstream_span_idx][1], seq_start +
                                            i] = counter_attn_scores[0, :, indices[downstream_span_idx][0]: indices[downstream_span_idx][1], seq_start + i]

                    patched_pattern = softmax(patched_attn_scores)
                    patched_pattern = torch.where(torch.isnan(
                        patched_pattern), torch.zeros_like(patched_pattern), patched_pattern)
                    z_patched_q = calculate_z_scores(
                        clean_v_act[layer_idx], patched_pattern)

                    # seq n_heads d
                    grad = grad_heads_output[layer_idx].clone()
                    results_clean = clean_head_results_act[layer_idx].clone()

                    result_patched_q = []
                    for i in range(z_patched_q.shape[0]):
                        result_patched_q.append(calculate_heads_output(
                            model, layer_idx, z_patched_q[i]))
                    if len(result_patched_q) > 0:
                        result_patched_q = torch.stack(result_patched_q, dim=0)
                    else:
                        result_patched_q = results_clean

                    attribute_score_q = grad * \
                        (result_patched_q - results_clean)
                    ...
                    for head_idx in range(model.cfg.n_heads):
                        ...
                        edge = Edge(upstream_layer_idx=layer_idx,
                                    ...
                                    is_crossing=True)
                        if indices[downstream_span_idx][1] - indices[downstream_span_idx][0] > 0:
                            attribute_score = attribute_score_q[:, indices[downstream_span_idx]
                                                                [0]:indices[downstream_span_idx][1], head_idx]
                            scores_q = {
                                "avg": attribute_score.sum(-1).mean().detach().to(torch.float32).cpu().numpy(),
                                "sum": attribute_score.sum().detach().to(torch.float32).cpu().numpy(),
...
if __name__ == "__main__":
    ...
    ap.add_argument("-ds", "--dataset_size", required=True,
                    type=int, help="dataset size")
